...
def encrypt_id(userid)
    pb_index = rand(1..6).to_s
    rotor_one_index = rand(1..6).to_s
    rotor_two_index = rand(1..6).to_s

    while rotor_one_index == rotor_two_index do
        rotor_one_index = rand(1..6).to_s
        rotor_two_index = rand(1..6).to_s
    end

    plug_board = $plug_boards[pb_index]
    rotor_one = $rotors[rotor_one_index]
    rotor_two = $rotors[rotor_two_index]

    userid = userid.split('')

    userid.each_with_index do |char, index|
...
def decrypt_id(userid)
    pb_index = userid[userid.length - 3].to_s
    rotor_one_index = userid[userid.length - 2].to_s
    rotor_two_index = userid[userid.length - 1].to_s
    ...
    userid.delete_at(userid.length - 1)
    userid.delete_at(userid.length - 1)
    userid.delete_at(userid.length - 1)
